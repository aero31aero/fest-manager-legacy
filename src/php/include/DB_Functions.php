...
class DB_Functions {
    ...
    public function regUser($name,$email,$phone,$college,$code,$reg,$accom,$team_id){
        ...
        $regUser=mysqli_query($GLOBALS['con'],"INSERT INTO users(name,email,phone,college,pearl_id,reg,accom,id_reg) VALUES('$name','$email','$phone','$college','$code','$reg','$accom','$team_id')");  
        if ($regUser) {
            // return user data
            $team_reg_collect_query=mysqli_query($GLOBALS['con'],"UPDATE dosh_credentials SET reg_collect=reg_collect+250 WHERE team_id='$team_id'");
            $response=array();
            $response['name']=$name;
    ...
    public function addUserToEvent($pearl_id,$event_id,$uploaded_by){
        ...
        $time=Date('U');
        $addUser=mysqli_query($GLOBALS['con'],"INSERT INTO event_participants(event_id,pearl_id,uploaded_by,updated_at) VALUES('$event_id','$pearl_id','$uploaded_by','$time')");
        if ($addUser) {
            // return user data
            $query=mysqli_query($GLOBALS['con'],"SELECT * FROM users WHERE pearl_id='$pearl_id'");
    ...
    mysqli_close($GLOBALS['con']);
     ...
     public function checkDoshLogin($team_id,$password){
        ...
        $checkUser=mysqli_query($GLOBALS['con'],"SELECT * FROM dosh_credentials WHERE team_id='$team_id' AND team_pass='$password'");
    ...
    public function getIndiEventData($event_id,$updated_at){
        ...
        $addUser=mysqli_query($GLOBALS['con'],"SELECT * FROM event_participants NATURAL JOIN users WHERE CAST(updated_at AS UNSIGNED) > $updated_at");
        if ($addUser) {
            // return user data
            $response=array();
            $i=0;
    ...
    public function getGroupEventData($event_id,$updated_at){
        ...
        $getGroupDetails=mysqli_query($GLOBALS['con'],"SELECT * FROM  group_details WHERE CAST(updated_at AS UNSIGNED) >$updated_at AND event_id='$event_id'");
        ...
        if ($getGroupDetails) {
            ...
            $getGroupMembers=mysqli_query($GLOBALS['con'],"SELECT * FROM group_members as gm,users as u WHERE gm.group_id IN (select group_id from group_details WHERE CAST(updated_at AS UNSIGNED) >$updated_at) and gm.pearl_id=u.pearl_id AND event_id='$event_id'");
            $response2=array();
            $i=0;
    ...
    public function checkEventAdmin($Bits_ID,$password){
        ...
        $checkEventAdmin=mysqli_query($GLOBALS['con'],"SELECT * FROM event_credentials WHERE Bits_ID='$Bits_ID' AND password='$password'");
    ...
    mysqli_close($GLOBALS['con']);
     ...
     public function getEvents(){
        ...
        $getEvents=mysqli_query($GLOBALS['con'],"SELECT * FROM pearl_events");
        if ($getEvents) {
            // return user data
            $response=array();
            $i=0;
    ...
    public function getSchedule($updated_at){
        ...
        $getEvents=mysqli_query($GLOBALS['con'],"SELECT * FROM event_details NATURAL JOIN pearl_events WHERE CAST(updated_at AS UNSIGNED) > $updated_at");
        if ($getEvents) {
            // return user data
            $response=array();
            $i=0;
    ...
    public function addUserToGroup($pearl_id,$group_id,$event_id){
        ...
        $addUser=mysqli_query($GLOBALS['con'],"INSERT INTO group_members(group_id,pearl_id,event_id) VALUES('$group_id','$pearl_id','$event_id')");
        if ($addUser) {
            $time=Date('U');
            $updatetime=mysqli_query($GLOBALS['con'],"UPDATE group_details SET updated_at='$time' WHERE group_id='$group_id' AND event_id='$event_id'");
            // return user data
            $query=mysqli_query($GLOBALS['con'],"SELECT * FROM users WHERE pearl_id='$pearl_id'");
            $result=mysqli_fetch_array($query);
            $response=array();
    ...
    public function updateParticipantRound($pearl_id,$event_id,$round_id){
        ...
        $addUser=mysqli_query($GLOBALS['con'],"UPDATE event_participants SET round_at='$round_id' WHERE event_id='$event_id' AND pearl_id='$pearl_id'");
        if ($addUser) {
            $response['event_id']=$event_id;
            $response['pearl_id']=$pearl_id;
            return $response;
    ...
    public function updateGroupRound($round_id,$group_id,$event_id){
        ...
        $time=Date('U');
        $addUser=mysqli_query($GLOBALS['con'],"UPDATE group_details SET round_at='$round_id',updated_at='$time' WHERE event_id='$event_id' AND group_id='$group_id'");
        if ($addUser) {
    ...
    public function getGroupId(){
        ...
        $getGroupId=mysqli_query($GLOBALS['con'],"SELECT DISTINCT group_id FROM group_details");
    ...
    public function addGroupDetails($group_id,$group_name,$event_id,$uploaded_by,$round_id){
        ...
        $time=Date('U');
        $addGroupDetails=mysqli_query($GLOBALS['con'],"INSERT INTO group_details(group_id,group_name,event_id,uploaded_by,round_at,updated_at) VALUES('$group_id','$group_name','$event_id','$uploaded_by','$round_id','$time')");
        if($addGroupDetails){
            $response=array();
            $response['event_id']=$event_id;
    ...
    public function addToEvent(){
        ...
        $addGroup=mysqli_query($GLOBALS['con'],"INSERT INTO events(event_id,user_id) VALUES('$event_id','$group_id')");
        if($addGroup){
            return true;
        }
        else{
...
public function getFeed($updated_at){
        ...
        $getFeed=mysqli_query($GLOBALS['con'],"SELECT * FROM notifications  WHERE CAST(updated_at AS UNSIGNED) > $updated_at");
        if ($getFeed) {
            // return user data
            $response=array();
            $i=0;
    ...
    public function updateNotification($message,$event_id){
        ...
        $time=Date('U');
        $addNotification=mysqli_query($GLOBALS['con'],"INSERT INTO notifications(event_id,message,updated_at) VALUES('$event_id','$message','$time')");
        if ($addNotification) {
            // return user data
            return true;
        } else {
